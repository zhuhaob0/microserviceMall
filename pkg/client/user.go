...
type UserClient interface {
	...
	CheckAdminUser(ctx context.Context, tracer opentracing.Tracer, request *pb.UserRequest) (*pb.UserResponse, error)
}

type UserClientImpl struct {
	/*
		可以配置负载均衡策略、重试、等机制。也可以配置invokeAfter和invokeBefore
	*/
...
func (impl *UserClientImpl) CheckAdminUser(ctx context.Context, tracer opentracing.Tracer, request *pb.UserRequest) (*pb.UserResponse, error) {
	response := new(pb.UserResponse)
	if err := impl.manager.DecoratorInvoke("/pb.UserService/AdminCheck", "admin_user_check", tracer, ctx, request, response); err == nil {
		return response, nil
	} else {
		return nil, err
	}
